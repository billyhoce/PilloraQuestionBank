...
6. **AWS S3** — create two buckets in `ap-southeast-1`:
   - `question-bank-prod` (images) and `question-bank-backups` (DB dumps).
   ...
   - Create one IAM user with `s3:GetObject`, `s3:PutObject`, `s3:DeleteObject` on `question-bank-prod`, plus `s3:PutObject` on `question-bank-backups`. Put its keys in `/opt/pillora/.env`.
...
| `S3_BUCKET` | Backend | Image bucket (`question-bank-prod`) |
